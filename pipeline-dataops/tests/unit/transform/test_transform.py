...
timezone_column = "utc_singapore_datetime"
...
def test_cast_columns(
    ...
    assert (
        ...
        == sample_raw_df[datetime_column].dt.tz_localize("UTC").dt.tz_convert(timezone)
    ).all()

    # check integer to datetime conversion
    for col in int_time_columns:
        assert (result_df[col] == pd.to_datetime(sample_raw_df[col], unit="ms")).all()

    # check float conversion
    for col in float_columns:
        assert (result_df[col] == sample_raw_df[col].astype(float)).all()
